...
from axiestudio.services.auth.utils import get_current_active_user, get_current_active_user_mcp
...
def get_suggestion_message(outdated_components: list[str]) -> str:
    ...
    if count == 0:
        return "Flödet innehåller inga föråldrade komponenter."
    if count == 1:
        return (
            "Flödet innehåller 1 föråldrad komponent. "
            f"Vi rekommenderar att uppdatera följande komponent: {outdated_components[0]}."
        )
    components = ", ".join(outdated_components)
    return (
        f"Flödet innehåller {count} föråldrade komponenter. Vi rekommenderar att uppdatera följande komponenter: {components}."
    )
...
async def verify_public_flow_and_get_user(flow_id: uuid.UUID, client_id: str | None) -> tuple[User, uuid.UUID]:
    ...
    async with session_scope() as session:
        ...
        from axiestudio.services.database.models.flow.model import AccessTypeEnum, Flow
