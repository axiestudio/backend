    ...
    for key, value in input_request.tweaks.items():
        ...
        if any(chat_key in key for chat_key in ("ChatInput", "Chat Input")):
            if request_has_input and input_request.input_type == "chat":
                msg = "Om du skickar ett input_value till chat-inmatningen kan du inte skicka en tweak med samma namn."
            ...
            msg = "Om du skickar ett input_value till textinmatningen kan du inte skicka en tweak med samma namn."
            raise InvalidChatInputError(msg)
...
async def simplified_run_flow(
    *,
    ...
    if flow is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Flöde hittades inte")
...
async def webhook_run_flow(
    ...
    try:
        ...
        if not data:
            error_msg = "Förfrågningskroppen är tom. Du bör tillhandahålla en JSON-payload som innehåller flödes-ID:t."
    ...
    return {"message": "Uppgift startad i bakgrunden", "status": "pågår"}
...
async def process(_flow_id) -> None:
    ...
    raise HTTPException(
        ...
        detail="Endpoint:en /process är föråldrad och kommer att tas bort i en framtida version. Vänligen använd /run istället.",
    )
...
async def get_task_status(_task_id: str) -> TaskStatusResponse:
    ...
    raise HTTPException(
        status_code=status.HTTP_400_BAD_REQUEST,
        detail="Endpoint:en /task är föråldrad och kommer att tas bort i en framtida version. Vänligen använd /run istället.",
    )
