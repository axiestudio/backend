...
from axiestudio.base.tools.constants import TOOL_OUTPUT_NAME
from axiestudio.io.schema import create_input_schema, create_input_schema_from_dict
    ...
    def get_tools(
        ...
        if len(tools) == 1 and (tool_name or tool_description):
            ...
            for tool in tools:
                ...
                tool.description = (
                    str(tool_description) + " Utdatadetaljer: " + str(tool.description)
                ) or tool.description
                tool.tags = [tool.name]
        elif tool_name or tool_description:
            msg = (
                "När ett verktygsnamn eller beskrivning skickas måste det bara finnas ett verktyg, "
                f"men {len(tools)} verktyg hittades."
            )
            raise ValueError(msg)
        return tools
