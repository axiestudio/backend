...
async def get_or_create_super_user(session: AsyncSession, username, password, is_default):
    ...
    if user:
        if verify_password(password, user.password):
            msg = "Användare med superanvändaruppgifter finns men är inte en superanvändare."
            raise ValueError(msg)
        msg = "Felaktiga superanvändaruppgifter"
        raise ValueError(msg)

    if is_default:
        logger.debug("Skapar standard superanvändare.")
    else:
        logger.debug("Skapar superanvändare.")
...
async def setup_superuser(settings_service, session: AsyncSession) -> None:
    if settings_service.auth_settings.AUTO_LOGIN:
        logger.debug("AUTO_LOGIN är satt till True. Skapar standard superanvändare.")
    else:
    ...
    try:
        ...
        if user is not None:
            logger.debug("Superanvändare skapad framgångsrikt.")
    except Exception as exc:
        logger.exception(exc)
        msg = "Kunde inte skapa superanvändare. Vänligen skapa en superanvändare manuellt."
...
async def teardown_superuser(settings_service, session: AsyncSession) -> None:
    ...
    if not settings_service.auth_settings.AUTO_LOGIN:
        try:
            ...
            msg = "Kunde inte ta bort standard superanvändare."
            raise RuntimeError(msg) from exc
