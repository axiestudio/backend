...
from axiestudio.inputs.inputs import HandleInput, IntInput, MultilineInput
...
class SQLGeneratorComponent(LCChainComponent):
    display_name = "Naturligt språk till SQL"
    description = "Generera SQL från naturligt språk."
    name = "SQLGenerator"
    legacy: bool = True
    icon = "LangChain"
    ...
    inputs = [
        ...
        IntInput(
            name="top_k",
            display_name="Topp K",
            info="Antalet resultat per select-sats att returnera.",
            value=5,
        ),
        MultilineInput(
            name="prompt",
            display_name="Prompt",
            info="Prompten måste innehålla `{question}`.",
        ),
    ]
