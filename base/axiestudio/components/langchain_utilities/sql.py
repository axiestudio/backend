...
class SQLAgentComponent(LCAgentComponent):
    ...
    description = "Konstruera en SQL-agent från en LLM och verktyg."
    name = "SQLAgent"
    icon = "LangChain"
    inputs = [
        *LCAgentComponent._base_inputs,
        HandleInput(name="llm", display_name="Språkmodell", input_types=["LanguageModel"], required=True),
        MessageTextInput(name="database_uri", display_name="Databas-URI", required=True),
        HandleInput(
            name="extra_tools",
            display_name="Extra verktyg",
            input_types=["Tool"],
            is_list=True,
            advanced=True,
        ),
