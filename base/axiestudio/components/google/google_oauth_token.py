...
from axiestudio.io import FileInput, MultilineInput, Output
from axiestudio.schema.data import Data


class GoogleOAuthToken(Component):
    display_name = "Google OAuth-token"
    description = "Genererar en JSON-sträng med din Google OAuth-token."
    ...
    inputs = [
        MultilineInput(
            name="scopes",
            display_name="Omfattningar",
            info="Inmatningsomfattningar för din applikation.",
            required=True,
        ),
        FileInput(
            name="oauth_credentials",
            display_name="Autentiseringsfil",
            info="Inmatning av OAuth-autentiseringsfil (t.ex. credentials.json).",
            file_types=["json"],
            required=True,
        ),
    ]
