...
from axiestudio.inputs.inputs import DropdownInput, MessageTextInput
...
class GoogleDriveSearchComponent(Component):
    display_name = "Google Drive-sökning"
    description = "Söker Google Drive-filer med angivna autentiseringsuppgifter och frågeparametrar."
    icon = "Google"
    legacy: bool = True
    ...
    inputs = [
        ...
        DropdownInput(
            ...
            info="Fältet att söka.",
            required=True,
        ),
        DropdownInput(
            name="valid_operator",
            display_name="Giltig operator",
            options=["contains", "=", "!=", "<=", "<", ">", ">=", "in", "has"],
            info="Operator att använda i frågan.",
            required=True,
        ),
        MessageTextInput(
            name="search_term",
            display_name="Sökordet",
            info="Värdet att söka efter i det angivna frågeobjektet.",
            required=True,
        ),
        MessageTextInput(
            name="query_string",
            display_name="Frågesträng",
            info="Frågesträngen som används för sökning. Du kan ändra detta manuellt.",
            value="",  # This will be updated with the generated query string
        ),
    ]

    outputs = [
        Output(display_name="Dokument-URL:er", name="doc_urls", method="search_doc_urls"),
        Output(display_name="Dokument-ID:n", name="doc_ids", method="search_doc_ids"),
        Output(display_name="Dokumenttitel", name="doc_titles", method="search_doc_titles"),
