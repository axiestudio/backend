...
class CouchbaseVectorStoreComponent(LCVectorStoreComponent):
    ...
    description = "Couchbase Vektorlager med sökmöjligheter"
    name = "Couchbase"
    icon = "Couchbase"

    inputs = [
        SecretStrInput(
            name="couchbase_connection_string", display_name="Couchbase Kluster-anslutningssträng", required=True
        ),
        StrInput(name="couchbase_username", display_name="Couchbase användarnamn", required=True),
        SecretStrInput(name="couchbase_password", display_name="Couchbase lösenord", required=True),
        StrInput(name="bucket_name", display_name="Bucket-namn", required=True),
        StrInput(name="scope_name", display_name="Scope-namn", required=True),
        StrInput(name="collection_name", display_name="Samlingnamn", required=True),
        StrInput(name="index_name", display_name="Indexnamn", required=True),
    ...
    def build_vector_store(self) -> CouchbaseVectorStore:
        try:
            ...
            msg = "Misslyckades med att importera Couchbase-beroenden. Installera det med `uv pip install axiestudio[couchbase] --pre`"
