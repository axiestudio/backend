...
from axiestudio.io import DropdownInput, HandleInput, IntInput, SecretStrInput, StrInput
from axiestudio.schema.data import Data


class PineconeVectorStoreComponent(LCVectorStoreComponent):
    display_name = "Pinecone"
    description = "Pinecone Vector Store med sökmöjligheter"
    name = "Pinecone"
    icon = "Pinecone"
    inputs = [
        StrInput(name="index_name", display_name="Indexnamn", required=True),
        StrInput(name="namespace", display_name="Namnrymd", info="Namnrymd för indexet."),
        DropdownInput(
            name="distance_strategy",
            display_name="Avståndsstrategi",
            options=["Cosine", "Euclidean", "Dot Product"],
            value="Cosine",
            advanced=True,
        ),
        SecretStrInput(name="pinecone_api_key", display_name="Pinecone API-nyckel", required=True),
        StrInput(
            name="text_key",
            display_name="Textnyckel",
            info="Nyckel i posten att använda som text.",
            value="text",
            advanced=True,
        ),
    ...
    def build_vector_store(self) -> VectorStore:
        ...
        try:
            ...
            msg = "langchain-pinecone är inte installerat. Vänligen installera det med `pip install langchain-pinecone`."
        ...
        try:
            ...
            error_msg = "Fel vid byggande av Pinecone vektorlager"
    ...
    def search_documents(self) -> list[Data]:
        ...
        try:
            ...
            error_msg = "Fel vid sökning av dokument"
            raise ValueError(error_msg) from e
        else:
