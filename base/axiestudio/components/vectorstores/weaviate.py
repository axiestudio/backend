...
from axiestudio.io import BoolInput, HandleInput, IntInput, SecretStrInput, StrInput
from axiestudio.schema.data import Data


class WeaviateVectorStoreComponent(LCVectorStoreComponent):
    display_name = "Weaviate"
    description = "Weaviate Vektorlager med sökmöjligheter"
    name = "Weaviate"
    icon = "Weaviate"

    inputs = [
        StrInput(name="url", display_name="Weaviate URL", value="http://localhost:8080", required=True),
        SecretStrInput(name="api_key", display_name="API-nyckel", required=False),
        StrInput(
            name="index_name",
            display_name="Indexnamn",
            required=True,
            info="Kräver indexnamn med stor bokstav.",
        ),
        StrInput(name="text_key", display_name="Textnyckel", value="text", advanced=True),
        *LCVectorStoreComponent.inputs,
        ...
        BoolInput(name="search_by_text", display_name="Sök efter text", advanced=True),
    ]
    ...
    def build_vector_store(self) -> Weaviate:
        ...
        if self.index_name != self.index_name.capitalize():
            msg = f"Weaviate kräver att indexnamnet börjar med stor bokstav. Använd: {self.index_name.capitalize()}"
