...
from axiestudio.inputs.inputs import IntInput, MultilineInput, NestedDictInput, SecretStrInput, StrInput
...
class GleanAPIWrapper(BaseModel):
    ...
    act_as: str = "axiestudio-component@datastax.com"  # TODO: Detect this
    ...
    def results(self, query: str, **kwargs: Any) -> list[dict[str, Any]]:
        ...
        if len(results) == 0:
            msg = "Inga bra Glean-sökresultat hittades"
    ...
    def run(self, query: str, **kwargs: Any) -> list[dict[str, Any]]:
        try:
            ...
            error_message = f"Fel i Glean Search API: {e!s}"
...
class GleanSearchAPIComponent(LCToolComponent):
    ...
    description: str = "Sök med Gleans API."
    documentation: str = "https://docs.axiestudio.se/Components/components-tools#glean-search-api"
    icon: str = "Glean"
    ...
    inputs = [
        ...
        SecretStrInput(name="glean_access_token", display_name="Glean-åtkomsttoken", required=True),
        MultilineInput(name="query", display_name="Fråga", required=True, tool_mode=True),
        IntInput(name="page_size", display_name="Sidstorlek", value=10),
        NestedDictInput(name="request_options", display_name="Begäransalternativ", required=False),
    ]
    ...
    def build_tool(self) -> Tool:
        ...
        tool = StructuredTool.from_function(
            ...
            description="Sök i Glean efter relevanta resultat.",
            func=wrapper.run,
            args_schema=GleanSearchAPISchema,
        )

        self.status = "Glean Search API-verktyg för Langchain"
