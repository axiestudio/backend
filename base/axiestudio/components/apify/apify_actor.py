...
from axiestudio.io import MultilineInput, Output, SecretStrInput, StrInput
...
class ApifyActorsComponent(Component):
    display_name = "Apify-aktörer"
    description = (
        "Använd Apify-aktörer för att extrahera data från hundratals platser snabbt. "
        "Denna komponent kan användas i ett flöde för att hämta data eller som ett verktyg med en agent."
    )
    documentation: str = "http://docs.axiestudio.org/integrations-apify"
    icon = "Apify"
    name = "ApifyActors"
    ...
    inputs = [
        SecretStrInput(
            ...
            info="API-tokenet för Apify-kontot.",
            required=True,
            password=True,
        ),
        StrInput(
