...
from axiestudio.base.data.docling_utils import extract_docling_documents
from axiestudio.custom import Component
from axiestudio.io import DropdownInput, HandleInput, MessageTextInput, Output, StrInput
from axiestudio.schema import Data, DataFrame


class ExportDoclingDocumentComponent(Component):
    display_name: str = "Exportera DoclingDocument"
    description: str = "Exportera DoclingDocument till markdown, HTML eller andra format."
    ...
    inputs = [
        HandleInput(
            name="data_inputs",
            display_name="Data eller DataFrame",
            info="Datan med dokument att exportera.",
            input_types=["Data", "DataFrame"],
            required=True,
        ),
        DropdownInput(
            name="export_format",
            display_name="Exportformat",
            options=["Markdown", "HTML", "Plaintext", "DocTags"],
            info="Välj exportformatet för att konvertera indata.",
            value="Markdown",
