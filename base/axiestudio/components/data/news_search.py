from urllib.parse import quote_plus

import pandas as pd
import requests
from bs4 import BeautifulSoup

from axiestudio.custom import Component
from axiestudio.io import IntInput, MessageTextInput, Output
from axiestudio.schema import DataFrame


class NewsSearchComponent(Component):
    display_name = "Nyhetssökning"
    description = "Söker Google News via RSS. Returnerar ren artikeldata."
    documentation: str = "https://docs.axiestudio.org/components-data#news-search"
    icon = "newspaper"
    name = "NewsSearch"

    inputs = [
        MessageTextInput(
            name="query",
            display_name="Sökfråga",
            info="Söknyckelord för nyhetsartiklar.",
            tool_mode=True,
            required=True,
        ),
        MessageTextInput(
            name="hl",
            display_name="Språk (hl)",
            info="Språkkod, t.ex. en-US, fr, de. Standard: en-US.",
            tool_mode=False,
            input_types=[],
            required=False,
            advanced=True,
        ),
        MessageTextInput(
            name="gl",
            display_name="Land (gl)",
            info="Landskod, t.ex. US, FR, DE. Standard: US.",
            tool_mode=False,
            input_types=[],
            required=False,
            advanced=True,
        ),
        MessageTextInput(
            name="ceid",
            display_name="Land:Språk (ceid)",
            info="t.ex. US:en, FR:fr. Standard: US:en.",
            tool_mode=False,
            value="US:en",
            input_types=[],
            required=False,
            advanced=True,
        ),
        MessageTextInput(
            name="topic",
            display_name="Ämne",
            info="Ett av: WORLD, NATION, BUSINESS, TECHNOLOGY, ENTERTAINMENT, SCIENCE, SPORTS, HEALTH.",
            tool_mode=False,
            input_types=[],
            required=False,
            advanced=True,
        ),
        MessageTextInput(
            name="location",
            display_name="Plats (Geo)",
            info="Stad, delstat eller land för platsbaserade nyheter. Lämna tomt för nyckelordssökning.",
            tool_mode=False,
            input_types=[],
            required=False,
            advanced=True,
        ),
        IntInput(
            name="timeout",
            display_name="Timeout",
            info="Timeout för förfrågan i sekunder.",
            value=5,
            required=False,
            advanced=True,
        ),
    ]

    outputs = [Output(name="articles", display_name="Nyhetsartiklar", method="search_news")]

    def search_news(self) -> DataFrame:
        # Defaults
        hl = getattr(self, "hl", None) or "en-US"
        gl = getattr(self, "gl", None) or "US"
        ceid = getattr(self, "ceid", None) or f"{gl}:{hl.split('-')[0]}"
        topic = getattr(self, "topic", None)
        location = getattr(self, "location", None)
        query = getattr(self, "query", None)

        # Build base URL
        if topic:
            # Topic-based feed
            base_url = f"https://news.google.com/rss/headlines/section/topic/{quote_plus(topic.upper())}"
            params = f"?hl={hl}&gl={gl}&ceid={ceid}"
            rss_url = base_url + params
        elif location:
            # Location-based feed
            base_url = f"https://news.google.com/rss/headlines/section/geo/{quote_plus(location)}"
            params = f"?hl={hl}&gl={gl}&ceid={ceid}"
            rss_url = base_url + params
        elif query:
            # Keyword search feed
            base_url = "https://news.google.com/rss/search?q="
            query_parts = [query]
            query_encoded = quote_plus(" ".join(query_parts))
            params = f"&hl={hl}&gl={gl}&ceid={ceid}"
            rss_url = f"{base_url}{query_encoded}{params}"
        else:
            self.status = "Ingen sökfråga, ämne eller plats angiven."
            self.log(self.status)
            return DataFrame(
                pd.DataFrame(
                    [
                        {
                            "title": "Fel",
                            "link": "",
                            "published": "",
                            "summary": "Ingen sökfråga, ämne eller plats angiven.",
                        }
                    ]
                )
            )

        try:
            response = requests.get(rss_url, timeout=self.timeout)
            response.raise_for_status()
            soup = BeautifulSoup(response.content, "xml")
            items = soup.find_all("item")
        except requests.RequestException as e:
            self.status = f"Misslyckades att hämta nyheter: {e}"
            self.log(self.status)
            return DataFrame(pd.DataFrame([{"title": "Fel", "link": "", "published": "", "summary": str(e)}]))
        except (AttributeError, ValueError, TypeError) as e:
            self.status = f"Oväntat fel: {e!s}"
            self.log(self.status)
            return DataFrame(pd.DataFrame([{"title": "Fel", "link": "", "published": "", "summary": str(e)}]))

        if not items:
            self.status = "Inga nyhetsartiklar hittades."
            self.log(self.status)
            return DataFrame(pd.DataFrame([{"title": "Inga artiklar hittades", "link": "", "published": "", "summary": ""}]))

        articles = []
        for item in items:
            try:
                title = self.clean_html(item.title.text if item.title else "")
                link = item.link.text if item.link else ""
                published = item.pubDate.text if item.pubDate else ""
                summary = self.clean_html(item.description.text if item.description else "")
                articles.append({"title": title, "link": link, "published": published, "summary": summary})
            except (AttributeError, ValueError, TypeError) as e:
                self.log(f"Fel vid tolkning av artikel: {e!s}")
                continue

        df_articles = pd.DataFrame(articles)
        self.log(f"Hittade {len(df_articles)} artiklar.")
        return DataFrame(df_articles)

    def clean_html(self, html_string: str) -> str:
        return BeautifulSoup(html_string, "html.parser").get_text(separator=" ", strip=True)
