...
class LangWatchComponent(Component):
    display_name: str = "LangWatch-utvärderare"
    description: str = "Utvärderar olika aspekter av språkmodeller med LangWatch:s utvärderingsslutpunkter."
    ...
    inputs = [
        DropdownInput(
            name="evaluator_name",
            display_name="Utvärderarnamn",
            options=[],
            required=True,
            info="Välj en utvärderare.",
            refresh_button=True,
        ...
        SecretStrInput(
            ...
            required=True,
            info="Ange din LangWatch API-nyckel.",
        ),
        MessageTextInput(
            name="input",
            display_name="Indata",
            required=False,
            info="Indatatexten för utvärdering.",
        ),
        MessageTextInput(
            name="output",
            display_name="Utdata",
            required=False,
            info="Utdatatexten för utvärdering.",
        ),
        MessageTextInput(
            name="expected_output",
            display_name="Förväntad utdata",
            required=False,
            info="Den förväntade utdatan för utvärdering.",
        ),
        MessageTextInput(
            name="contexts",
            display_name="Kontexter",
            required=False,
            info="Kontexterna för utvärdering (kommaseparerade).",
        ),
        IntInput(
            name="timeout",
            display_name="Timeout",
            info="Maximal tid (i sekunder) som tillåts för servern att svara innan timeout.",
            value=30,
            advanced=True,
        ),
    ]
    ...
    outputs = [
        Output(name="evaluation_result", display_name="Utvärderingsresultat", method="evaluate"),
    ]
