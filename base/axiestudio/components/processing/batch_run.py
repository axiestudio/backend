...
class BatchRunComponent(Component):
    display_name = "Batch-körning"
    description = "Kör en LLM på varje rad i en DataFrame-kolumn. Om ingen kolumn anges används alla kolumner."
    documentation: str = "https://docs.axiestudio.org/components-processing#batch-run"
    icon = "List"
    ...
    inputs = [
        HandleInput(
            ...
            info="Anslut 'Språkmodell'-utdata från din LLM-komponent här.",
            input_types=["LanguageModel"],
            required=True,
        ),
        MultilineInput(
            name="system_message",
            display_name="Instruktioner",
            info="Flerradiga systeminstruktioner för alla rader i DataFrame.",
            required=False,
        ),
        DataFrameInput(
            name="df",
            display_name="DataFrame",
            info="DataFrame vars kolumn (specificerad av 'column_name') vi kommer att behandla som textmeddelanden.",
            required=True,
        ),
        MessageTextInput(
            name="column_name",
