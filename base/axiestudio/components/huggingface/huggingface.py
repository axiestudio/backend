...
from axiestudio.io import DictInput, DropdownInput, FloatInput, IntInput, SecretStrInput, SliderInput, StrInput
...
class HuggingFaceEndpointsComponent(LCModelComponent):
    ...
    description: str = "Generera text med Hugging Face Inference API:er."
    icon = "HuggingFace"
    ...
    inputs = [
        *LCModelComponent._base_inputs,
        DropdownInput(
            ...
            info="Välj en modell från Hugging Face Hub",
            options=[
                DEFAULT_MODEL,
        ...
        StrInput(
            name="custom_model",
            display_name="Anpassad modell-ID",
            info="Ange en anpassad modell-ID från Hugging Face Hub",
            value="",
            show=False,
            required=True,
        ),
        IntInput(
            name="max_new_tokens", display_name="Max nya tokens", value=512, info="Maximalt antal genererade tokens"
        ),
        IntInput(
            name="top_k",
            display_name="Top K",
            advanced=True,
            info="Antalet ordförrådstoken med högst sannolikhet att behålla för top-k-filtrering",
        ),
        FloatInput(
            name="top_p",
            display_name="Top P",
            ...
            advanced=True,
            info=(
                "Om satt till < 1, behålls endast den minsta uppsättningen av mest sannolika tokens med "
                "sannolikheter som summerar till `top_p` eller högre för generering"
            ),
        ),
        FloatInput(
            name="typical_p",
            display_name="Typisk P",
            value=0.95,
            advanced=True,
            info="Typisk avkodningsmassa.",
        ),
        SliderInput(
            name="temperature",
            ...
            info="Värdet som används för att modulera logits-fördelningen",
            advanced=True,
        ),
        FloatInput(
            name="repetition_penalty",
            display_name="Upprepningsstraff",
            info="Parametern för upprepningsstraff. 1.0 betyder inget straff.",
            advanced=True,
        ),
        StrInput(
        ...
        DropdownInput(
            ...
            advanced=True,
            info="Uppgiften att anropa modellen med. Bör vara en uppgift som returnerar `generated_text` eller `summary_text`.",
        ),
        SecretStrInput(name="huggingfacehub_api_token", display_name="API-token", password=True, required=True),
        DictInput(name="model_kwargs", display_name="Modellnyckelord-argument", advanced=True),
        IntInput(name="retry_attempts", display_name="Försök igen", value=1, advanced=True),
    ]
        ...
        if "huggingface" in self.inference_endpoint.lower():
            if self.model_id == "custom":
                if not self.custom_model:
                    error_msg = "Anpassad modell-ID krävs när 'custom' är valt"
    ...
    def build_model(self) -> LanguageModel:
        ...
        try:
            ...
            msg = "Kunde inte ansluta till Hugging Face Endpoints API."
