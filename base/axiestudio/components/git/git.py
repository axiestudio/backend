...
class GitLoaderComponent(Component):
    display_name = "Git"
    description = (
        "Ladda och filtrera dokument från ett lokalt eller fjärr-Git-repository. "
        "Använd en lokal repo-sökväg eller klona från en fjärr-URL."
    )
    trace_type = "tool"
    icon = "GitLoader"
    ...
    inputs = [
        DropdownInput(
            name="repo_source",
            display_name="Repository-källa",
            options=["Local", "Remote"],
            required=True,
            info="Välj om du vill använda en lokal repo-sökväg eller klona från en fjärr-URL.",
            real_time_refresh=True,
        ),
        MessageTextInput(
            name="repo_path",
            display_name="Lokal repository-sökväg",
            required=False,
    ...
    async def temp_clone_dir(self):
        ...
        temp_dir = None
        try:
            temp_dir = tempfile.mkdtemp(prefix="axiestudio_clone_")
            yield temp_dir
        finally:
            if temp_dir:
