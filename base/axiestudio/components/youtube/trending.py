...
from axiestudio.inputs.inputs import BoolInput, DropdownInput, IntInput, SecretStrInput
...
class YouTubeTrendingComponent(Component):
    ...
    display_name: str = "YouTube-trender"
    description: str = "Hämtar trendande videor från YouTube med filtreringsalternativ."
    icon: str = "YouTube"
    ...
    inputs = [
        ...
        DropdownInput(
            ...
            value="Global",
            info="Regionen att hämta trendande videor från.",
        ),
        DropdownInput(
            name="category",
            ...
            value="All",
            info="Kategorin av videor att hämta.",
        ),
        IntInput(
            name="max_results",
            display_name="Max resultat",
            value=10,
            info="Maximalt antal trendande videor att returnera (1-50).",
        ),
        BoolInput(
            name="include_statistics",
            display_name="Inkludera statistik",
            value=True,
            info="Inkludera videostatistik (visningar, gillningar, kommentarer).",
        ),
        BoolInput(
            name="include_content_details",
            display_name="Inkludera innehållsdetaljer",
            value=True,
            info="Inkludera videolängd och kvalitetsinformation.",
            advanced=True,
        ),
        BoolInput(
            name="include_thumbnails",
            display_name="Inkludera miniatyrbilder",
            value=True,
            info="Inkludera video-miniatyrbild-URL:er.",
            advanced=True,
        ),
    ]
    ...
    def get_trending_videos(self) -> DataFrame:
        ...
        try:
            ...
            error_message = f"YouTube API-fel: {e}"
            if e.resp.status == HTTP_FORBIDDEN:
                error_message = "API-kvot överskriden eller åtkomst förbjuden."
            elif e.resp.status == HTTP_NOT_FOUND:
                error_message = "Resurs hittades inte."
            ...
            logging.exception("Ett oväntat fel uppstod:")
