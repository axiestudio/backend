...
class GoogleSerperAPICore(Component):
    ...
    description = "Anropar Serper.dev Google Search API och hämtar resultaten."
    icon = "Serper"

    inputs = [
        SecretStrInput(
            name="serper_api_key",
            display_name="Serper API-nyckel",
            required=True,
        ),
        MultilineInput(
            name="input_value",
    ...
    def search_serper(self) -> DataFrame:
        try:
            ...
            error_message = f"Ett fel uppstod vid sökning: {e!s}"
    ...
    def text_search_serper(self) -> Message:
        ...
        text_result = search_results.to_string(index=False) if not search_results.empty else "Inga resultat hittades."
