...
class PythonCodeStructuredTool(LCToolComponent):
    ...
    display_name = "Python-kod strukturerad"
    description = "Strukturerat verktyg dataclass-kod till verktyg"
    ...
    inputs = [
        MultilineInput(
            name="tool_code",
            display_name="Verktygskod",
            info="Ange dataclass-koden.",
        ...
        MessageTextInput(
            ...
            info="Ange namnet på verktyget.",
            required=True,
        ),
        MessageTextInput(
            name="tool_description",
            display_name="Beskrivning",
            info="Ange beskrivningen av verktyget.",
            required=True,
        ),
        BoolInput(
            name="return_direct",
            display_name="Returnera direkt",
            info="Ska verktyget returnera funktionsutdata direkt?",
        ),
        DropdownInput(
            name="tool_function",
            display_name="Verktygsfunktion",
            info="Välj funktionen för ytterligare uttryck.",
            options=[],
            required=True,
            real_time_refresh=True,
            refresh_button=True,
        ...
        HandleInput(
            ...
            display_name="Globala variabler",
            info="Ange de globala variablerna eller skapa datakomponent.",
            input_types=["Data"],
    ...
    async def update_build_config(
        ...
        try:
            ...
            for func in functions:
                ...
                for arg in func["args"]:
                    ...
                    field = MessageTextInput(
                        display_name=f"{arg['name']}: Beskrivning",
                        name=field_name,
                        info=f"Ange beskrivningen för {arg['name']}",
                        required=True,
                    )
            ...
            self.status = f"Misslyckades att extrahera namn: {e}"
            logger.opt(exception=True).debug(self.status)
            build_config["tool_function"]["options"] = ["Misslyckades att tolka", str(e)]
        return build_config
