...
class VectaraVectorStoreComponent(LCVectorStoreComponent):
    ...
    description: str = "Vectara vektorlager med sökmöjligheter"
    name = "Vectara"
    icon = "Vectara"

    inputs = [
        StrInput(name="vectara_customer_id", display_name="Vectara kund-ID", required=True),
        StrInput(name="vectara_corpus_id", display_name="Vectara corpus-ID", required=True),
        SecretStrInput(name="vectara_api_key", display_name="Vectara API-nyckel", required=True),
        HandleInput(
            name="embedding",
    ...
    def _add_documents_to_vector_store(self, vector_store: Vectara) -> None:
        """Lägg till dokument till vektorlager."""
