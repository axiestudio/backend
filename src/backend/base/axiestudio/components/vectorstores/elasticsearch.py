...
class ElasticsearchVectorStoreComponent(LCVectorStoreComponent):
    """Elasticsearch Vektorlager med avancerade, anpassningsbara sökmöjligheter."""

    display_name: str = "Elasticsearch"
    description: str = "Elasticsearch Vektorlager med avancerade, anpassningsbara sökmöjligheter."
    name = "Elasticsearch"
    ...
    inputs = [
        StrInput(
            ...
            info="URL för självhanterade Elasticsearch-distributioner (t.ex. http://localhost:9200). "
            "Använd inte med Elastic Cloud-distributioner, använd Elastic Cloud ID istället.",
        ),
        SecretStrInput(
            name="cloud_id",
            display_name="Elastic Cloud ID",
            value="",
            info="Använd detta för Elastic Cloud-distributioner. Använd inte tillsammans med 'Elasticsearch URL'.",
        ),
        StrInput(
            name="index_name",
            display_name="Indexnamn",
            value="axiestudio",
            info="Indexnamnet där vektorerna kommer att lagras i Elasticsearch-klustret.",
        ),
        *LCVectorStoreComponent.inputs,
        StrInput(
            ...
            advanced=False,
            info=(
                "Elasticsearch användarnamn (t.ex. 'elastic'). "
                "Krävs för både lokala och Elastic Cloud-inställningar om inte API-nycklar används."
            ),
        ),
        SecretStrInput(
            name="password",
            ...
            advanced=False,
            info=(
                "Elasticsearch lösenord för den angivna användaren. "
                "Krävs för både lokala och Elastic Cloud-inställningar om inte API-nycklar används."
            ),
        ),
        HandleInput(
        ...
        FloatInput(
            ...
            display_name="Sökresultattröskel",
            info="Minsta likhetströskel för sökresultat.",
            value=0.0,
            advanced=True,
        ),
        SecretStrInput(
            name="api_key",
            display_name="Elastic API-nyckel",
            value="",
            advanced=True,
            info="API-nyckel för Elastic Cloud-autentisering. Om den används krävs inte 'användarnamn' och 'lösenord'.",
        ),
        BoolInput(
            name="verify_certs",
            display_name="Verifiera SSL-certifikat",
            value=True,
            advanced=True,
            info="Om SSL-certifikat ska verifieras vid anslutning till Elasticsearch.",
        ),
    ]
    ...
    def build_vector_store(self) -> ElasticsearchStore:
        ...
        if self.cloud_id and self.elasticsearch_url:
            msg = (
                "Både 'cloud_id' och 'elasticsearch_url' angivna. "
                "Vänligen använd endast en baserat på din distribution (Cloud eller Lokal)."
            )
