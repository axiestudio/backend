...
class QdrantVectorStoreComponent(LCVectorStoreComponent):
    display_name = "Qdrant"
    description = "Qdrant Vector Store med sökmöjligheter"
    icon = "Qdrant"

    inputs = [
        StrInput(name="collection_name", display_name="Samlingsnamn", required=True),
        StrInput(name="host", display_name="Värd", value="localhost", advanced=True),
        IntInput(name="port", display_name="Port", value=6333, advanced=True),
        IntInput(name="grpc_port", display_name="gRPC-port", value=6334, advanced=True),
        SecretStrInput(name="api_key", display_name="API-nyckel", advanced=True),
        StrInput(name="prefix", display_name="Prefix", advanced=True),
        IntInput(name="timeout", display_name="Timeout", advanced=True),
        StrInput(name="path", display_name="Sökväg", advanced=True),
        StrInput(name="url", display_name="URL", advanced=True),
        DropdownInput(
            name="distance_func",
            display_name="Avståndsfunktion",
            options=["Cosine", "Euclidean", "Dot Product"],
            value="Cosine",
            advanced=True,
        ),
        StrInput(name="content_payload_key", display_name="Innehållsnyckel", value="page_content", advanced=True),
        StrInput(name="metadata_payload_key", display_name="Metadatanyckel", value="metadata", advanced=True),
    ...
    def build_vector_store(self) -> Qdrant:
        ...
        if not isinstance(self.embedding, Embeddings):
            msg = "Ogiltigt inbäddningsobjekt"
            raise TypeError(msg)
