...
from axiestudio.inputs import DataInput, DropdownInput, MessageInput, MultilineInput, SecretStrInput, SliderInput
from axiestudio.io import Output
from axiestudio.schema.message import Message


class TaskError(Exception):
    """Fel som uppstår när en uppgift misslyckas."""


class TaskTimeoutError(Exception):
    """Fel som uppstår när en uppgift får timeout."""


class IndexCreationError(Exception):
    """Fel som uppstår när det finns ett problem med ett index."""
    ...
    def process_video(self) -> Message:
        ...
        try:
            ...
            if not video_path or not Path(video_path).exists():
                return Message(text="Ogiltig videosökväg")

            if not self.api_key:
                return Message(text="Ingen API-nyckel angiven")
            ...
            try:
                ...
                return Message(text=f"Misslyckades med att hämta/skapa index: {e}")
