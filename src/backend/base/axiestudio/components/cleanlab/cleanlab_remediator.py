...
from axiestudio.io import BoolInput, FloatInput, HandleInput, MessageTextInput, Output, PromptInput
...
class CleanlabRemediator(Component):
    ...
    display_name = "Cleanlab-åtgärdare"
    description = (
        "Åtgärdar ett opålitligt svar baserat på förtroendepoäng från Cleanlab-utvärderaren, "
        "poängtröskel och meddelandehanteringsinställningar."
    )
    icon = "Cleanlab"
    ...
    inputs = [
        MessageTextInput(
            name="response",
            display_name="Svar",
            info="Svaret på användarens fråga.",
            required=True,
        ),
        HandleInput(
            name="score",
            display_name="Förtroendepoäng",
            info="Förtroendepoängen från Cleanlab-utvärderaren.",
            input_types=["number"],
            required=True,
        ),
        MessageTextInput(
            name="explanation",
            display_name="Förklaring",
            info="Förklaringen från Cleanlab-utvärderaren.",
            required=False,
        ),
        FloatInput(
            name="threshold",
            display_name="Tröskel",
            field_type="float",
            value=0.7,
