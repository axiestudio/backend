...
from axiestudio.io import BoolInput, FileInput, MessageTextInput, Output
from axiestudio.schema.dataframe import DataFrame


class BigQueryExecutorComponent(Component):
    display_name = "BigQuery"
    description = "Utför SQL-frågor på Google BigQuery."
    ...
    inputs = [
        FileInput(
            ...
            display_name="Ladda upp tjänstekonto-JSON",
            info="Ladda upp JSON-filen som innehåller Google Cloud-tjänstekontoautentiseringsuppgifter.",
            file_types=["json"],
            required=True,
        ),
        MessageTextInput(
            name="query",
            display_name="SQL-fråga",
            info="SQL-frågan att utföra på BigQuery.",
            required=True,
            tool_mode=True,
        ),
        BoolInput(
            name="clean_query",
            display_name="Rensa fråga",
            info="När aktiverad kommer detta automatiskt att rensa din SQL-fråga.",
            value=False,
            advanced=True,
        ),
    ]
