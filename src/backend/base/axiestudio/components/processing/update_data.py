...
class UpdateDataComponent(Component):
    display_name: str = "Uppdatera data"
    description: str = "Uppdatera eller lägg till data dynamiskt med de angivna fälten."
    name: str = "UpdateData"
    ...
    def validate_text_key(self, data: Data) -> None:
        ...
        if self.text_key and self.text_key not in data_keys:
            msg = f"Textnyckel: '{self.text_key}' hittades inte i Data-nycklarna: {', '.join(data_keys)}"
            raise ValueError(msg)
