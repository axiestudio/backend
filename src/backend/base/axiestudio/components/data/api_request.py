...
from axiestudio.inputs.inputs import TabInput
...
from axiestudio.utils.component_utils import set_current_fields, set_field_advanced, set_field_display
...
class APIRequestComponent(Component):
    display_name = "API-förfrågan"
    description = "Gör HTTP-förfrågningar med URL eller cURL-kommandon."
    documentation: str = "https://docs.axiestudio.se/components-data#api-request"
    icon = "Globe"
    name = "APIRequest"
    ...
    inputs = [
        MessageTextInput(
            name="url_input",
            display_name="URL",
            info="Ange URL:en för förfrågan.",
            advanced=False,
            tool_mode=True,
        ),
        MultilineInput(
            ...
            display_name="cURL",
            info=(
                "Klistra in ett curl-kommando för att fylla i fälten. "
                "Detta kommer att fylla i ordlistefälten för rubriker och kropp."
            ),
        ...
        TabInput(
            ...
            options=["URL", "cURL"],
            value="URL",
            info="Aktivera cURL-läge för att fylla i fält från ett cURL-kommando.",
            real_time_refresh=True,
        ),
        DataInput(
            name="query_params",
            display_name="Frågeparametrar",
            info="Frågeparametrarna att lägga till URL:en.",
            advanced=True,
        ),
        TableInput(
            name="body",
            display_name="Kropp",
            info="Kroppen att skicka med förfrågan som en ordlista (för POST, PATCH, PUT).",
            table_schema=[
                {
                    "name": "key",
                    "display_name": "Nyckel",
        ...
        IntInput(
            ...
            value=30,
            info="Timeout att använda för förfrågan.",
            advanced=True,
        ),
        BoolInput(
            name="follow_redirects",
            display_name="Följ omdirigeringar",
            value=True,
            info="Om HTTP-omdirigeringar ska följas.",
            advanced=True,
        ),
        BoolInput(
            name="save_to_file",
            display_name="Spara till fil",
            value=False,
            info="Spara API-svaret till en temporär fil",
            advanced=True,
        ),
        BoolInput(
            name="include_httpx_metadata",
            display_name="Inkludera HTTPx-metadata",
            value=False,
            info=(
                "Inkludera egenskaper som rubriker, statuskod, svarsrubriker, "
                "och omdirigeringshistorik i utdata."
            ),
            advanced=True,
        ),
    ]
    ...
    outputs = [
        Output(display_name="API-svar", name="data", method="make_api_request"),
    ]
    ...
    def _process_list_body(self, body: list) -> dict:
        ...
        try:
            ...
            self.log(f"Misslyckades att bearbeta kroppslista: {e}")
            return {}
    ...
    def parse_curl(self, curl: str, build_config: dotdict) -> dotdict:
        ...
        try:
            ...
            msg = f"Fel vid tolkning av curl: {exc}"
            self.log(msg)
    ...
    async def make_request(
        ...
        *,
        ...
        if method not in {"GET", "POST", "PATCH", "PUT", "DELETE"}:
            msg = f"Metod som inte stöds: {method}"
            raise ValueError(msg)
        ...
        try:
            ...
            if is_binary:
                ...
                try:
                    ...
                    self.log("Misslyckades att avkoda JSON-svar")
            ...
            self.log(f"Fel vid förfrågan till {url}")
            return Data(
                data={
                    "source": url,
                    "headers": headers,
