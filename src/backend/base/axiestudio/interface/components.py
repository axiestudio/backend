...
async def import_axiestudio_components():
    """Asynchronously discovers and loads all built-in Axie Studio components with module-level parallelization.

    Scans the `axiestudio.components` package and its submodules in parallel, instantiates classes that are subclasses
    ...
    modules_dict = {}
    try:
        import axiestudio.components as components_pkg
    except ImportError as e:
        logger.error(f"Failed to import axiestudio.components package: {e}", exc_info=True)
...
def _process_single_module(modname: str) -> tuple[str, dict] | None:
    ...
    try:
        ...
        return None
    # Extract the top-level subpackage name after "axiestudio.components."
    # e.g., "axiestudio.components.Notion.add_content_to_page" -> "Notion"
...
async def get_and_cache_all_types_dict(
    ...
    Supports both full and partial (lazy) loading. If the cache is empty, loads built-in Axie Studio
    ...
    if component_cache.all_types_dict is None:
        ...
        axiestudio_components = await import_axiestudio_components()
        custom_components_dict = await _determine_loading_strategy(settings_service)

        # merge the dicts
        component_cache.all_types_dict = {
            **axiestudio_components["components"],
...
async def get_component_minimal_metadata(component_type: str, component_name: str, components_paths: list[str]):
    ...
    metadata = {
        ...
        "name": component_name,
        "type": component_type,
        "description": f"En {component_type} komponent (inte helt laddad)",
        "template": {
            "_type": component_type,
            "inputs": {},
            "outputs": {},
            "output_types": [],
            "documentation": f"En {component_type} komponent",
            "display_name": component_name.replace("_", " ").title(),
...
async def load_single_component(component_type: str, component_name: str, components_paths: list[str]):
    ...
    from axiestudio.custom.utils import get_single_component_dict
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from axiestudio.custom.utils import build_custom_components
