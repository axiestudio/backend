...
from axiestudio.cli.progress import create_axiestudio_progress
from axiestudio.initial_setup.setup import get_or_create_default_folder
from axiestudio.logging.logger import configure, logger
from axiestudio.main import setup_app
from axiestudio.services.database.utils import session_getter
from axiestudio.services.deps import get_db_service, get_settings_service, session_scope
from axiestudio.services.settings.constants import DEFAULT_SUPERUSER
...
def run(
    *,
    ...
    env_log_level = os.environ.get("AXIESTUDIO_LOG_LEVEL")
    if env_log_level is None:
        os.environ["AXIESTUDIO_LOG_LEVEL"] = log_level_str
    else:
        os.environ["AXIESTUDIO_LOG_LEVEL"] = env_log_level.lower()
    ...
    progress = create_axiestudio_progress(verbose=verbose)
    ...
    with progress.step(1):
        for key, value in os.environ.items():
            new_key = key.replace("AXIESTUDIO_", "")
    ...
    if platform.system() == "Windows":
        ...
        with progress.step(6):
            # Use Gunicorn with Axie StudioUvicornWorker for non-Windows systems
            from axiestudio.server import AxieStudioApplication
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    with suppress(httpx.ConnectError):
        ...
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if axiestudio_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use axiestudio.utils.version to get the version info
    version_info = get_version_info()
    axiestudio_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= axiestudio_is_pre_release(axiestudio_version)  # Update pre-release status

    notice = build_version_notice(axiestudio_version, package_name)
    ...
    telemetry_text = (
        (
            "We collect anonymous usage data to improve Axie Studio.\n"
            "To opt out, set: [bold]DO_NOT_TRACK=true[/bold] in your environment."
        )
        if os.getenv("DO_NOT_TRACK", os.getenv("AXIESTUDIO_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve Axie Studio.\n"
    ...
    try:
        ...
        try:
            ...
            logger.info("Website: https://www.axiestudio.se")
...
def superuser(
    ...
    log_level: str = typer.Option("error", help="Logging level.", envvar="AXIESTUDIO_LOG_LEVEL"),
) -> None:
    """Create a superuser."""
...
def copy_db() -> None:
    ...
    This function copies the 'axiestudio.db' and 'axiestudio-pre.db' files from the cache directory to the current
    directory.
    ...
    db_path = cache_dir / "axiestudio.db"
    pre_db_path = cache_dir / "axiestudio-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from axiestudio.services.database.models.api_key.crud import create_api_key, delete_api_key
            from axiestudio.services.database.models.api_key.model import ApiKey, ApiKeyCreate
