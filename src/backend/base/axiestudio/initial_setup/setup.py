...
from axiestudio.base.constants import (
...
from axiestudio.initial_setup.constants import STARTER_FOLDER_DESCRIPTION, STARTER_FOLDER_NAME
...
from axiestudio.services.database.models.folder.constants import DEFAULT_FOLDER_NAME
from axiestudio.services.database.models.folder.model import Folder, FolderCreate, FolderRead
from axiestudio.services.database.models.user.crud import get_user_by_username
from axiestudio.services.deps import get_settings_service, get_storage_service, get_variable_service, session_scope
from axiestudio.template.field.prompt import DEFAULT_PROMPT_INTUT_TYPES
from axiestudio.utils.util import escape_json_dump
...
async def load_flows_from_directory() -> None:
    ...
    if not settings_service.auth_settings.AUTO_LOGIN:
        logger.warning("AUTO_LOGIN är inaktiverad, laddar inte flöden från katalog")
        return

    async with session_scope() as session:
        user = await get_user_by_username(session, settings_service.auth_settings.SUPERUSER)
        if user is None:
            msg = "Superanvändare hittades inte i databasen"
...
async def load_bundles_from_urls() -> tuple[list[TemporaryDirectory], list[str]]:
    ...
    if not bundle_urls:
        return [], []
    if not settings_service.auth_settings.AUTO_LOGIN:
        logger.warning("AUTO_LOGIN är inaktiverad, laddar inte flöden från URL:er")
...
async def initialize_super_user_if_needed() -> None:
    ...
    if not username or not password:
        logger.debug("SUPERUSER och SUPERUSER_PASSWORD inte satta - hoppar över skapande av superanvändare")
        return
    ...
    logger.debug("Superanvändare initialiserad")
