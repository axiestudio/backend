...
from axiestudio.schema.content_types import ErrorContent
from axiestudio.schema.data import Data
from axiestudio.schema.image import Image, get_file_paths, is_image_file
from axiestudio.schema.properties import Properties, Source
from axiestudio.schema.validators import timestamp_to_str, timestamp_to_str_validator
...
from axiestudio.utils.image import create_image_content_dict
...
class Message(Data):
    ...
    def to_lc_message(
        ...
        if self.text is None or not self.sender:
            logger.warning("Saknar obligatoriska nycklar ('text', 'sender') i meddelande, använder HumanMessage som standard.")
    ...
    def to_dataframe(self) -> DataFrame:
        from axiestudio.schema.dataframe import DataFrame  # Local import to avoid circular import
...
class DefaultModel(BaseModel):
    ...
    def custom_encoder(obj):
        ...
        msg = f"Objekt av typ {obj.__class__.__name__} är inte JSON-serialiserbart"
        raise TypeError(msg)
...
class MessageResponse(DefaultModel):
    ...
    def from_message(cls, message: Message, flow_id: str | None = None):
        ...
        if message.text is None or not message.sender or not message.sender_name:
            msg = "Meddelandet har inte de obligatoriska fälten (text, sender, sender_name)."
            raise ValueError(msg)
        return cls(
...
class ErrorMessage(Message):
    """En meddelandeklass specifikt för felmeddelanden med fördefinierade felspecifika attribut."""
    ...
    def _format_markdown_reason(exception: BaseException) -> str:
        ...
        if hasattr(exception, "body") and isinstance(exception.body, dict) and "message" in exception.body:
            ...
            reason += " - **Ett okänt fel inträffade.**\n"
        return reason
