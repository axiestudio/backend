...
export default function UpdateComponentModal({
  ...
  const { handleDuplicate } = useDuplicateFlows({
    flow: currentFlow
      ? { ...currentFlow, name: currentFlow.name + " (Säkerhetskopia)" }
      : undefined,
  });
  ...
  return (
    ...
    >
      ...
      <BaseModal.Header>
        <span className="">
          Uppdatera{" "}
          {isMultiple ? "komponenter" : (components?.[0]?.display_name ?? "")}
        </span>
      </BaseModal.Header>
      <BaseModal.Content overflowHidden>
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-3 text-sm text-muted-foreground">
            {isMultiple ? (
              <p>
                Uppdateringar markerade som{" "}
                <span className="font-semibold text-accent-amber-foreground">
                  brytande
                </span>{" "}
                kan ändra inmatningar, utmatningar eller komponentbeteende. I vissa
                fall kommer de att koppla bort komponenter från ditt flöde, vilket kräver
                you to review or reconnect them afterward. Components added from
                sidofältet använder alltid den senaste versionen.
              </p>
            ) : (
              <>
                <p>
                  Denna uppdatering kan ändra inmatningar, utmatningar eller komponentbeteende.
                  I vissa fall kommer den att{" "}
                  <span className="font-semibold text-accent-amber-foreground">
                    koppla bort denna komponent från ditt flöde
                  </span>
                  , vilket kräver att du granskar eller återansluter den efteråt.
                </p>
                <p>
          ...
          >
            ...
            <label htmlFor="backupFlow" className="cursor-pointer select-none">
              Skapa säkerhetskopia av flöde före uppdatering
            </label>
          </div>
        </div>
      </BaseModal.Content>
      <BaseModal.Footer
        submit={{
          label: "Uppdatera komponent" + (components.length > 1 ? "er" : ""),
