...
export default function ShareModal({
  ...
  const handleShareComponent = async (update = false) => {
    ...
    function successShare() {
      ...
      setSuccessData({
        title: `${is_component ? "Komponent" : "Flöde"} delat framgångsrikt!`,
      });
    }
    ...
    if (!update)
      saveFlowStore(
        ...
        setErrorData({
          title: "Fel vid delning av " + (is_component ? "komponent" : "flöde"),
      ...
      <>
        ...
        >
          <ConfirmationModal.Content>
            <span>
              Det verkar som att {name} redan finns. Vill du ersätta den med den
              nuvarande?
            </span>
            <br></br>
            <span className="text-xs text-destructive">
              Obs: Denna åtgärd är irreversibel.
            </span>
          </ConfirmationModal.Content>
        </ConfirmationModal>
      </>
  ...
  return (
    <>
      ...
      >
        ...
        <BaseModal.Header
          description={`Publicera ${
            is_component ? "din komponent" : "arbetsflöde"
          } till Axie Studio Store.`}
        >
          <span className="pr-2">Dela</span>
          <IconComponent
            name="Share3"
          ...
          {open && (
            <>
              ...
              <div className="mb-2 mt-5 flex items-center space-x-2">
                ...
                >
                  Sätt {nameComponent === "component" ? "komponent" : "arbetsflöde"} status till offentlig
                </label>
              </div>
              <span className="text-xs text-destructive">
                <b>Observera:</b> API-nycklar i specificerade fält tas automatiskt
                bort vid delning.
              </span>
            </>
          )}
        ...
        <BaseModal.Footer
          submit={{
            label: `Dela ${is_component ? "komponent" : "flöde"}`,
