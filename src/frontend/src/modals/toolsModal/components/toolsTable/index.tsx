  ...
  const columnDefs: ColDef[] = [
    {
      ...
      headerName: isAction ? "Flödesnamn" : "Namn",
      flex: 1,
      valueGetter: (params) =>
        !isAction
          ? parseString(
    ...
    {
      field: "name",
      headerName: isAction ? "Verktyg" : "Slug",
      flex: 1,
      resizable: false,
      valueGetter: (params) =>
        params.data.name !== ""
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <Input
                    ...
                    maxLength={46}
                    placeholder="Redigera namn..."
                    data-testid="input_update_name"
                  />
                  <div className="text-xs text-muted-foreground">
                    {isAction
                      ? "Används som funktionsnamn när detta flöde exponeras för klienter."
                      : "Används som funktionsnamn när detta verktyg exponeras för agenten."}
                  </div>
                </div>
                <div className="flex flex-col gap-2">
                  <label
                  ...
                  >
                    {isAction ? "Verktygsbeskrivning" : "Beskrivning"}
                  </label>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {isAction
                      ? "Detta är beskrivningen för verktyget som exponeras för en klient."
                      : "Detta är beskrivningen för verktyget som exponeras för agenterna."}
                  </div>
                </div>
              </div>
            ) : (
                    ...
                    {actionArgs.length > 0 && (
                      <div className="flex flex-col gap-1.5">
                        ...
                        <p className="text-mmd text-muted-foreground">
                          Hantera inmatningar för detta verktyg
                        </p>
                      </div>
                    )}
                    {actionArgs.map((field, index) => (
                      <div key={index} className="flex flex-col gap-2">
                        ...
                        <Input
                          id="sidebar-desc-input"
                          disabled
                          placeholder="Inmatning kontrollerad av agenten"
                          onChange={(e) => {}}
                        />
                      </div>
                    ))}
