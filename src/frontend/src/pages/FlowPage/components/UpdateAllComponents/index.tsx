...
const ERROR_MESSAGE_UPDATING_COMPONENTS = "Fel vid uppdatering av komponenter";
const ERROR_MESSAGE_UPDATING_COMPONENTS_LIST = [
  "Det uppstod ett fel vid uppdatering av komponenterna.",
  "Om felet kvarstår, vänligen kontrollera din konfiguration och försök igen.",
];
const ERROR_MESSAGE_EDGES_LOST =
  "Vissa kanter förlorades efter uppdatering av komponenterna. Vänligen granska flödet och återanslut dem.";
...
export default function UpdateAllComponents() {
  ...
  const getSuccessTitle = (updatedCount: number) => {
    resetEdgesUpdateRef();
    return `Framgångsrikt uppdaterade ${updatedCount} komponent${
      updatedCount > 1 ? "er" : ""
    }`;
  };
          ...
          <div className="absolute bottom-2 left-1/2 z-50 w-[530px] -translate-x-1/2">
            ...
            >
              <div className="flex items-center gap-3">
                <span>
                  Uppdatering
                  {componentsToUpdateFiltered.length > 1 ? "ar är" : " är"}{" "}
                  tillgänglig för{" "}
                  {componentsToUpdateFiltered.length +
                    " komponent" +
                    (componentsToUpdateFiltered.length > 1 ? "er" : "")}
                </span>
              </div>
              <div className="flex items-center gap-4">
                ...
                >
                  Avfärda {componentsToUpdateFiltered.length > 1 ? "alla" : ""}
                </Button>
                <Button
                  size="sm"
                  className="shrink-0"
                ...
                >
                  {breakingChanges.length > 0 ? "Granska alla" : "Uppdatera alla"}
                </Button>
              </div>
