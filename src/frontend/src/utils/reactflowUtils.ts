 ...
 *
 * This file contains the highest number of commits by Otávio in the entire Axie Studio project,
...
export function validateNode(node: AllNodeType, edges: Edge[]): Array<string> {
  if (!node.data?.node?.template || !Object.keys(node.data.node.template)) {
    return [
      "Vi har upptäckt ett potentiellt problem med en komponent i flödet. Vänligen granska det och, om nödvändigt, skicka in en felrapport med din exporterade flödesfil. Tack för din hjälp!",
    ];
  }
  ...
  return Object.keys(template).reduce((errors: Array<string>, t) => {
    if (
      ...
      errors.push(
        `${displayName || type} saknar ${getFieldTitle(template, t)}.`,
      );
    } else if (
      ...
      if (hasDuplicateKeys(template[t].value))
        errors.push(
          `${displayName || type} (${getFieldTitle(
            template,
            t,
          )}) innehåller dubblettnycklar med samma värden.`,
        );
    ...
    return [
      {
        id: "",
        errors: [
          "Inga komponenter hittades i flödet. Vänligen lägg till minst en komponent till flödet.",
        ],
      },
    ];
  }
...
export function validateSelection(
  ...
  if (clonedSelection.nodes.length < 2) {
    errorsArray.push("Vänligen välj mer än en komponent");
  }
  if (
    ...
    errorsArray.push("Välj endast icke-inmatnings/utmatningskomponenter");
  }
  ...
  if (
    ...
    errorsArray.push("Välj endast en komponent med fria utmatningar");
  }
  ...
  if (
    ...
    errorsArray.push("Välj endast anslutna komponenter");
  }
  return errorsArray;
}
function updateGroupNodeTemplate(template: APITemplateType) {
  ...
  Object.keys(template).forEach((key) => {
    ...
    if (
      AXIESTUDIO_SUPPORTED_TYPES.has(type) &&
