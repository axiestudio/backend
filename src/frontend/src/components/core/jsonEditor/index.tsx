  ...
  const handleTransform = (isInitial = false) => {
    ...
    try {
        ...
        if (result !== undefined) {
          ...
          if (isValidResult(result)) {
            try {
              ...
              setErrorData({
                title: "Ogiltigt Resultat",
                list: [
                  "Det filtrerade resultatet innehåller värden som inte kan serialiseras till JSON",
                ],
              });
              return;
            }
            ...
            setErrorData({
              title: "Ogiltigt Resultat",
              list: [
                "Det filtrerade resultatet måste vara ett JSON-objekt eller array, inte ett primitivt värde",
              ],
            });
            return;
          }
      ...
      for (const key of path) {
        if (result === undefined || result === null) {
          setErrorData({
            ...
            list: [`Sökväg '${transformQuery}' ledde till odefinierat eller null-värde`],
          });
          return;
        }
        if (Array.isArray(result)) {
          ...
          if (indexMatch) {
            ...
            if (index >= result.length) {
              setErrorData({
                title: "Ogiltigt Array-Index",
                list: [
                  `Index ${index} är utanför gränserna för array med längd ${result.length}`,
                ],
              });
              return;
            }
          ...
          result = result
            .map((item) => {
              if (!(key in item)) {
                setErrorData({
                  title: "Ogiltig Egenskap",
                  list: [`Egenskap '${key}' finns inte i array-objekt`],
                });
                return undefined;
              }
              return item[key];
          ...
          if (!(key in result)) {
            setErrorData({
              title: "Ogiltig Egenskap",
              list: [`Egenskap '${key}' finns inte i objekt`],
            });
            return;
          }
          result = result[key];
      ...
      if (result !== undefined) {
        ...
        setErrorData({
          title: "Ogiltigt Resultat",
          list: ["Transformering resulterade i odefinierat värde"],
        });
      }
    } catch (error) {
      console.error("Error applying transform:", error);
      setErrorData({
        title: "Transformeringsfel",
