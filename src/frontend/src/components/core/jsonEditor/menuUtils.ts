    ...
    onClick: () => {
      ...
      if (!editor) {
        setErrorData({
          ...
          list: ["Redigerare inte tillgänglig"],
        });
        return;
      }
      ...
      navigator.clipboard
        ...
        .then(() => {
          setSuccessData({ title: "JSON kopierat till urklipp" });
        })
        .catch(() => {
          setErrorData({
            title: "Kopiering misslyckades",
            list: ["Kunde inte kopiera till urklipp. Vänligen kopiera manuellt."],
          });
        });
    },
    icon: faCopy,
    title: "Kopiera JSON till urklipp",
  };
};
...
export const enhanceExistingCopyButtons = (
  items: MenuItem[],
  setSuccessData: (data: { title: string }) => void,
  successMessage: string = "JSON kopierat till urklipp",
): MenuItem[] => {
